            ...
            if (calcChance(level, chance.getValue().doubleValue(), chance.isAmpEffect())) {
                ...
                for (Entity nearbyEntity : nearbyEntities) {
                    if(nearbyEntities instanceof Player){
                        ...
                        if (instance.getByPlayer(nearbyPlayer).getRelationTo(byPlayer).isAtLeast(Relation.ALLY)) {
                            ...
                            player.setHealth(Math.min(player.getMaxHealth(), health+(amt*(ampEffect ? level : 1))));
